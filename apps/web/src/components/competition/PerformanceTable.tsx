...
//   );
// }
// apps/web/src/components/competition/PerformanceTable.tsx (ORIGINAL + EXPAND/COLLAPSE)
'use client';
...
// Componente de célula compacta com tooltip (ORIGINAL SEM MUDANÇAS)
const CompactCell = ({
  criterio,
  dados,
}: {
...
// Componente de célula de total (ORIGINAL SEM MUDANÇAS)
const TotalCell = ({
  criterionName,
  total,
  type,
...
export default function PerformanceTable({
  ...
  const toggleCard = (sectorId: number) => {
  ...
  // Transformar dados para ordenação por pontuação total (ORIGINAL SEM MUDANÇAS)
  const dataWithRanking = useMemo(() => {
    if (!resultsBySector || Object.keys(resultsBySector).length === 0) {
      return [];
    }
  ...
  // Calcular totais por critério (ORIGINAL SEM MUDANÇAS)
  const criterionTotals = useMemo(() => {
    if (!uniqueCriteria || dataWithRanking.length === 0) {
      return {};
    }
  ...
  // Componente de Card para Mobile (ORIGINAL + EXPAND/COLLAPSE)
  const MobileSectorCard = ({
    setor,
    posicao,
  }: {
    ...
    const isFirstPlace = posicao === 1;
    const isExpanded = expandedCards.has(setor.sectorId);

    return (
      <div
        className={`p-4 mb-4 rounded-lg shadow-md cursor-pointer ${isFirstPlace ? 'bg-green-50 border-2 border-green-200' : 'bg-white border border-gray-200'}`}
        onClick={() => toggleCard(setor.sectorId)}
      >
        <div className='flex items-center justify-between mb-3 border-b pb-2'>
          <div className='flex items-center gap-2'>
            <span className='text-xl font-bold'>{getPosicaoIcon(posicao)}</span>
        ...
        {/* CONTEÚDO EXPANDIDO - SÓ APARECE QUANDO CLICADO */}
        {isExpanded && (
          <div className='grid grid-cols-1 gap-4'>
            {uniqueCriteria.map((criterio) => {
              const dados = setor.criteriaResults[criterio.id];
  ...
  // Componente de Card de Totais para Mobile (ORIGINAL SEM MUDANÇAS)
  const MobileTotalsCard = () => (
    <div className='bg-blue-100 border-2 border-blue-300 p-4 rounded-lg shadow-md mt-4'>
      <h3 className='font-bold text-lg mb-3 text-blue-900 border-b border-blue-300 pb-2'>
        📊 Totais por Critério
      ...
      <div className='space-y-3'>
        {uniqueCriteria.map((criterio) => {
          ...
          const fontSizeClass = isSpecialCriterion ? 'text-sm' : 'text-base';
          const formattedValue = formatNumberByCriterion(
            totalData.value,
            criterio.nome
          );
  ...
  return (
    <TooltipProvider>
      <div className='w-full'>
        {isMobile ? (
          // LAYOUT PARA MOBILE (CARDS) - ORIGINAL + EXPAND/COLLAPSE
          <>
            <div className='mb-4'>
              <p className='text-sm text-gray-600'>
                Período Atual • Menor pontuação = Melhor posição
          ...
          // LAYOUT ORIGINAL PARA DESKTOP (TABELA) - ZERO MUDANÇAS
          <>
            <div className='mb-4'>
              <p className='text-sm text-gray-600'>
                Período Atual • Menor pontuação = Melhor posição
