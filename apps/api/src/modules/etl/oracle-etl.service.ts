...
export class OracleEtlService {
  ...
  > {
    ...
    try {
      ...
      const query = `
        SELECT
            COALESCE(K.CIDADE, C.CIDADE) AS CODIGOGA, -- Pega o código da garagem
            COALESCE(K.DATA_MES_ANO, C.DATA_MES_ANO) AS DATA_MES_ANO, -- Pega o mês/ano
            NVL(K.KM_OPERACIONAL, 0) AS KM,
            NVL(C.QTDECOMB, 0) AS QNTCOMB
        FROM
            ( /* Subquery K: KM Operacional por Garagem/Mês */
              SELECT L.CODIGOGA AS CIDADE, SUM(NVL(GLOBUS.FC_ARR_KMBCO_VIAGENS(BC.IDBCO, 0 ,BV.IDBCOVIAGENS), 0)) AS KM_OPERACIONAL, TRUNC(BC.DATABCO, 'mm') AS DATA_MES_ANO FROM T_ARR_BCO BC JOIN T_ARR_BCO_VIAGENS BV ON BC.IDBCO = BV.IDBCO JOIN BGM_CADLINHAS L ON BV.CODINTLINHA = L.CODINTLINHA WHERE BC.DATABCO BETWEEN TO_DATE(:1, 'YYYY-MM-DD') AND TO_DATE(:2, 'YYYY-MM-DD') + 0.99999 AND BC.CODIGOEMPRESA = 4 AND L.CODIGOGA IN (31, 124, 239, 240) GROUP BY L.CODIGOGA, TRUNC(BC.DATABCO, 'mm')
            ) K
            FULL OUTER JOIN
            ( /* Subquery C: Combustível Quantidade por Garagem/Mês */
              SELECT V.CODIGOGA AS CIDADE, SUM(NVL(C.QTDECOMB,0)) AS QTDECOMB, TRUNC(C.DATAABASTCARRO, 'mm') AS DATA_MES_ANO FROM VWABA_CONSCOMBREPVEIC C JOIN FRT_CADVEICULOS V ON C.CODIGOVEIC=V.CODIGOVEIC WHERE C.DATAABASTCARRO BETWEEN TO_DATE(:1, 'YYYY-MM-DD') AND TO_DATE(:2, 'YYYY-MM-DD') + 0.99999 AND V.CODIGOGA IN (31, 124, 239, 240) AND V.CODIGOEMPRESA = 4 GROUP BY V.CODIGOGA, TRUNC(C.DATAABASTCARRO, 'mm')
            ) C ON K.CIDADE = C.CIDADE AND K.DATA_MES_ANO = C.DATA_MES_ANO
        ORDER BY CODIGOGA, DATA_MES_ANO
        `;
