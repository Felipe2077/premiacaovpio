...
const start = async () => {
  try {
    ...
    // === 3. REGISTRAR PLUGIN DE AUTENTICAÇÃO ===
    await fastify.register(authPlugin);
    fastify.log.info('✅ Plugin de autenticação registrado.');

    // === 4. REGISTRAR ROTAS DE AUTENTICAÇÃO ===
    ...
    // === 9. ROTAS DE CRITÉRIOS E SETORES (PROTEGIDAS) ===
    fastify.get(
      '/api/criteria/active',
      {
        preHandler: [fastify.authenticate, viewReports],
    ...
    // === 10. ROTAS DE PERÍODOS DE COMPETIÇÃO (PROTEGIDAS) ===
    fastify.get(
      '/api/periods/active',
      {
        preHandler: [fastify.authenticate, viewReports],
    ...
    // === 11. ROTAS DE PARÂMETROS (PROTEGIDAS) ===
    fastify.get(
      '/api/parameters',
      {
        preHandler: [
      ...
      async (request, reply) => {
        ...
        if (!queryParams.period) {
          return reply.status(400).send({
            message:
              "Query parameter 'period' (formato YYYY-MM) é obrigatório.",
          });
        }
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        if (isNaN(parameterId)) {
          return reply
            .status(400)
            .send({ message: 'ID do parâmetro inválido.' });
        }
      ...
      async (request, reply) => {
        ...
        try {
          if (!request.body || typeof request.body !== 'object') {
            return reply.status(400).send({
              error:
                'Body da requisição é obrigatório e deve ser um objeto JSON',
            });
          }
    ...
    // === 14. ROTAS DE RELATÓRIOS E ESTATÍSTICAS DE EXPURGO ===
    fastify.get(
    ...
    fastify.post(
      ...
      async (request, reply) => {
        try {
          ...
          fastify.log.error('Erro ao criar usuário:', error);
          reply.status(500).send({ error: 'Erro interno' });
        }
      }
    );

    // === 17. ROTAS DE TESTE PARA PERMISSÕES ===
    fastify.get(
    ...
    fastify.get(
      ...
      async (request, reply) => {
        return {
          message: 'Área restrita - apenas diretores',
          user: request.user,
          timestamp: new Date().toISOString(),
        };
      }
