...
export class OracleDataService {
  ...
  // NOVAS consultas específicas para metas operacionais
  async fetchDailyOperationalData(
    startDate: string,
    endDate: string,
    sectorIds: number[]
  ): Promise<DailyOperationalData[]> {
    const oracleConnection = await this.oracleDataSource.manager;

    // SQL ESPECÍFICO para KM diária (baseado nos SQLs da documentação)
    const query = `
      SELECT 
        ...
        CASE 
          WHEN TO_CHAR(data_operacao, 'D') = '1' THEN 'DOMINGO_FERIADO'
          WHEN TO_CHAR(data_operacao, 'D') = '7' THEN 'SABADO'
          ELSE 'UTIL'
        END as tipo_dia_automatico
      FROM oracle_operacao_frota
      WHERE data_operacao >= TO_DATE(:startDate, 'YYYY-MM-DD')
        AND data_operacao <= TO_DATE(:endDate, 'YYYY-MM-DD')
        AND codigo_setor IN (124, 239, 240, 31)
    ...
    const results = await oracleConnection.query(query, {
      startDate,
      endDate,
    });

    // MAPEAR códigos Oracle → IDs da aplicação
    return this.mapOracleResultsToApplicationData(results);
  }
...
- Implementar consultas SQL específicas
...
✅ Serviços base implementados:
   ...
   - OracleDataService (consultas específicas)
   - OracleDataService (consultas básicas)
   - HolidayManagementService (detecção de feriados)
   - OracleDataValidationService

✅ Consultas SQL Oracle específicas:
   - fetchDailyOperationalData()
   - fetchMonthlyConsolidatedData()
   - fetchVehicleCountBySector()
   - calculateAnnualAverageCosts()

✅ Validações de dados Oracle:
   - validateDataCompleteness()
   - validateCostConsistency()
   - detectDataAnomalies()

✅ Mapeamento Oracle ↔ Sistema:
   - OracleMappingService
   - Testes de mapeamento

✅ Scripts de migração do banco
✅ Testes unitários dos serviços base
✅ Scripts de teste Oracle específicos
...
### **14.5: Consultas SQL Específicas para Oracle**

```sql
-- ============================================================================
-- CONSULTAS SQL ESPECÍFICAS PARA CÁLCULO DE METAS OPERACIONAIS
-- Estas consultas são NOVAS e específicas para nossa funcionalidade
-- ============================================================================

-- -----------------------------------------------------------------------------
-- 1. OPERAÇÃO DIÁRIA PARA KM PREVISTA (3 meses históricos)
-- -----------------------------------------------------------------------------
-- Descrição: Buscar KM rodada diária por setor com classificação automática de tipo de dia
-- Uso: Cálculo das médias históricas para KM PREVISTA
-- Mapeamento: códigos Oracle (124,239,240,31) → setores da aplicação

SELECT
    TO_CHAR(op.data_operacao, 'YYYY-MM-DD') as data_operacao,
    op.codigo_setor,
    SUM(op.km_rodada) as km_rodada_dia,
    CASE
        WHEN TO_CHAR(op.data_operacao, 'D') = '1' THEN 'DOMINGO_FERIADO'  -- Domingo
        WHEN TO_CHAR(op.data_operacao, 'D') = '7' THEN 'SABADO'           -- Sábado
        ELSE 'UTIL'                                                        -- Dias úteis
    END as tipo_dia_automatico
FROM oracle_schema.t_operacao_frota op
WHERE op.data_operacao >= TO_DATE(:startDate, 'YYYY-MM-DD')
  AND op.data_operacao <= TO_DATE(:endDate, 'YYYY-MM-DD')
  AND op.codigo_setor IN (124, 239, 240, 31)
  AND op.status_registro = 'ATIVO'
  AND op.km_rodada > 0  -- Filtrar registros inválidos
GROUP BY op.data_operacao, op.codigo_setor
ORDER BY op.data_operacao DESC, op.codigo_setor;

-- -----------------------------------------------------------------------------
-- 2. CONSOLIDADOS MENSAIS (3 meses para COMBUSTÍVEL)
-- -----------------------------------------------------------------------------
-- Descrição: Dados mensais agregados para cálculo de eficiência de combustível
-- Uso: Média de eficiência dos últimos 3 meses (KM/L)

SELECT
    TO_CHAR(op.data_operacao, 'YYYY-MM') as ano_mes,
    op.codigo_setor,
    SUM(op.km_rodada) as total_km_mes,
    SUM(cb.litros_consumidos) as total_litros_mes,
    COUNT(DISTINCT op.data_operacao) as dias_operacao,
    AVG(op.km_rodada) as media_km_dia
FROM oracle_schema.t_operacao_frota op
INNER JOIN oracle_schema.t_consumo_combustivel cb ON (
    cb.codigo_setor = op.codigo_setor
    AND cb.data_consumo = op.data_operacao
)
WHERE TO_CHAR(op.data_operacao, 'YYYY-MM') IN (:mes1, :mes2, :mes3)
  AND op.codigo_setor IN (124, 239, 240, 31)
  AND op.status_registro = 'ATIVO'
  AND cb.litros_consumidos > 0
GROUP BY TO_CHAR(op.data_operacao, 'YYYY-MM'), op.codigo_setor
ORDER BY ano_mes DESC, op.codigo_setor;

-- -----------------------------------------------------------------------------
-- 3. CUSTOS ANUAIS PARA PNEUS E PEÇAS (12 meses - Anexo A)
-- -----------------------------------------------------------------------------
-- Descrição: Calcular custos médios anuais R$/km para PNEUS e PEÇAS
-- Uso: Valor base para cálculo das metas de PNEUS e PEÇAS

WITH dados_anuais AS (
    SELECT
        op.codigo_setor,
        cs.tipo_custo,
        SUM(op.km_rodada) as total_km_anual,
        SUM(cs.valor_custo) as total_custo_anual,
        COUNT(DISTINCT TO_CHAR(op.data_operacao, 'YYYY-MM')) as meses_com_dados
    FROM oracle_schema.t_operacao_frota op
    INNER JOIN oracle_schema.t_custos_operacionais cs ON (
        cs.codigo_setor = op.codigo_setor
        AND cs.data_custo = op.data_operacao
    )
    WHERE op.data_operacao >= ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -12)
      AND op.data_operacao < TRUNC(SYSDATE, 'MM')  -- Até mês anterior fechado
      AND cs.tipo_custo IN ('PNEUS', 'PECAS')
      AND op.codigo_setor IN (124, 239, 240, 31)
      AND op.status_registro = 'ATIVO'
      AND cs.valor_custo > 0
      AND op.km_rodada > 0
    GROUP BY op.codigo_setor, cs.tipo_custo
)
SELECT
    codigo_setor,
    tipo_custo,
    ...
    meses_com_dados,
    ROUND(total_custo_anual / NULLIF(total_km_anual, 0), 6) as custo_medio_rs_por_km,
    -- Validação: deve ter pelo menos 10 meses de dados
    CASE WHEN meses_com_dados >= 10 THEN 'VALIDO' ELSE 'INSUFICIENTE' END as status_dados
FROM dados_anuais
WHERE total_km_anual > 0
  AND meses_com_dados >= 6  -- Mínimo 6 meses para ser considerado
ORDER BY codigo_setor, tipo_custo;

-- -----------------------------------------------------------------------------
-- 4. CADASTRO DA FROTA ATIVA (Contagem de Veículos)
-- -----------------------------------------------------------------------------
-- Descrição: Contagem atual de veículos ativos por setor
-- Uso: Cálculo de meta por veículo (informativo)

SELECT
    cf.codigo_setor,
    COUNT(*) as qtd_veiculos_total,
    COUNT(CASE WHEN cf.status_veiculo = 'OPERACIONAL' THEN 1 END) as qtd_operacional,
    COUNT(CASE WHEN cf.status_veiculo = 'MANUTENCAO' THEN 1 END) as qtd_manutencao,
    MAX(cf.data_ultima_atualizacao) as ultima_atualizacao_frota
FROM oracle_schema.t_cadastro_frota cf
WHERE cf.status_registro = 'ATIVO'
  AND cf.codigo_setor IN (124, 239, 240, 31)
  AND cf.data_desativacao IS NULL
GROUP BY cf.codigo_setor
HAVING COUNT(*) > 0  -- Garantir que o setor tem pelo menos 1 veículo
ORDER BY cf.codigo_setor;

-- -----------------------------------------------------------------------------
-- 5. DADOS PARA SISTEMA DE SALDO (Mês Anterior)
-- -----------------------------------------------------------------------------
-- Descrição: Buscar gastos reais do mês anterior para cálculo do saldo devedor
-- Uso: Sistema de Saldo para PNEUS e PEÇAS

SELECT
    cs.codigo_setor,
    cs.tipo_custo,
    TO_CHAR(cs.data_custo, 'YYYY-MM') as mes_referencia,
    SUM(cs.valor_custo) as gasto_real_mes,
    COUNT(DISTINCT cs.data_custo) as dias_com_custos,
    AVG(cs.valor_custo) as custo_medio_diario,
    MAX(cs.data_custo) as ultima_data_custo
FROM oracle_schema.t_custos_operacionais cs
WHERE TO_CHAR(cs.data_custo, 'YYYY-MM') = :mes_anterior
  AND cs.tipo_custo IN ('PNEUS', 'PECAS')
  AND cs.codigo_setor IN (124, 239, 240, 31)
  AND cs.valor_custo > 0
GROUP BY cs.codigo_setor, cs.tipo_custo, TO_CHAR(cs.data_custo, 'YYYY-MM')
ORDER BY cs.codigo_setor, cs.tipo_custo;

-- -----------------------------------------------------------------------------
-- 6. VALIDAÇÃO DE CONSISTÊNCIA DOS DADOS
-- -----------------------------------------------------------------------------
-- Descrição: Verificar integridade e consistência dos dados antes do cálculo
-- Uso: Validações pré-cálculo

-- 6.1 Verificar se todos os setores têm dados no período
SELECT
    codigo_setor,
    COUNT(DISTINCT TO_CHAR(data_operacao, 'YYYY-MM-DD')) as dias_com_dados,
    MIN(data_operacao) as primeira_data,
    MAX(data_operacao) as ultima_data,
    AVG(km_rodada) as media_km_dia,
    CASE
        WHEN COUNT(DISTINCT TO_CHAR(data_operacao, 'YYYY-MM-DD')) < 60 THEN 'INSUFICIENTE'
        WHEN AVG(km_rodada) < 100 THEN 'SUSPEITO_BAIXO'
        WHEN AVG(km_rodada) > 50000 THEN 'SUSPEITO_ALTO'
        ELSE 'OK'
    END as status_consistencia
FROM oracle_schema.t_operacao_frota
WHERE data_operacao >= TO_DATE(:startDate, 'YYYY-MM-DD')
  AND data_operacao <= TO_DATE(:endDate, 'YYYY-MM-DD')
  AND codigo_setor IN (124, 239, 240, 31)
  AND status_registro = 'ATIVO'
...
-- 6.2 Verificar custos médios dentro de faixas esperadas
SELECT
    tipo_custo,
    codigo_setor,
    custo_medio_rs_por_km,
    CASE
        WHEN tipo_custo = 'PNEUS' AND custo_medio_rs_por_km BETWEEN 0.10 AND 2.00 THEN 'OK'
        WHEN tipo_custo = 'PECAS' AND custo_medio_rs_por_km BETWEEN 0.50 AND 5.00 THEN 'OK'
        ELSE 'FORA_FAIXA'
    END as status_faixa
FROM (
    -- Subconsulta dos custos médios calculados anteriormente
    SELECT tipo_custo, codigo_setor,
           total_custo_anual / NULLIF(total_km_anual, 0) as custo_medio_rs_por_km
    FROM dados_anuais
    WHERE total_km_anual > 0
)
ORDER BY tipo_custo, codigo_setor;
```

---

### **14.6: Mapeamento Oracle ↔ Sistema**

```typescript
// ============================================================================
// MAPEAMENTO DE CÓDIGOS ORACLE PARA ENTIDADES DA APLICAÇÃO
// ============================================================================

interface OracleSectorMapping {
  oracleCode: number;
  sectorId: number;
  sectorName: string;
  isActive: boolean;
...
// Mapeamento oficial Oracle → Sistema
export const ORACLE_SECTOR_MAPPING: OracleSectorMapping[] = [
  { oracleCode: 124, sectorId: 3, sectorName: 'SANTA MARIA', isActive: true },
  { oracleCode: 239, sectorId: 4, sectorName: 'SÃO SEBASTIÃO', isActive: true },
  { oracleCode: 240, sectorId: 1, sectorName: 'GAMA', isActive: true },
  { oracleCode: 31, sectorId: 2, sectorName: 'PARANOÁ', isActive: true },
];

// Funções de conversão
export class OracleMappingService {
  static getApplicationSectorId(oracleCode: number): number | null {
    const mapping = ORACLE_SECTOR_MAPPING.find(
      (m) => m.oracleCode === oracleCode
    );
    return mapping?.sectorId || null;
  }

  static getOracleCode(sectorId: number): number | null {
    const mapping = ORACLE_SECTOR_MAPPING.find((m) => m.sectorId === sectorId);
    return mapping?.oracleCode || null;
  }

  static getAllOracleCodes(): number[] {
    return ORACLE_SECTOR_MAPPING.filter((m) => m.isActive).map(
      (m) => m.oracleCode
    );
  }

  static getAllSectorIds(): number[] {
    return ORACLE_SECTOR_MAPPING.filter((m) => m.isActive).map(
      (m) => m.sectorId
    );
  }

  static validateMapping(): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    // Verificar duplicatas Oracle
    const oracleCodes = ORACLE_SECTOR_MAPPING.map((m) => m.oracleCode);
    const uniqueOracle = new Set(oracleCodes);
    if (oracleCodes.length !== uniqueOracle.size) {
      errors.push('Códigos Oracle duplicados encontrados');
    }

    // Verificar duplicatas Sistema
    const sectorIds = ORACLE_SECTOR_MAPPING.map((m) => m.sectorId);
    const uniqueSectors = new Set(sectorIds);
    if (sectorIds.length !== uniqueSectors.size) {
      errors.push('IDs de setor duplicados encontrados');
    }
...
### **14.7: Scripts de Validação de Dados**

```typescript
// ============================================================================
// SCRIPTS DE VALIDAÇÃO PARA DADOS ORACLE
// ============================================================================

export class OracleDataValidationService {
  /**
   * Valida se os dados Oracle estão completos para o período
   */
  async validateDataCompleteness(
    startDate: string,
    endDate: string
  ): Promise<DataValidationReport> {
    const results = await this.oracleConnection.query(
      `
      WITH date_range AS (
        SELECT TO_DATE(:startDate, 'YYYY-MM-DD') + LEVEL - 1 as check_date
        FROM DUAL
      ...
      data_availability AS (
        SELECT 
          ...
          sm.codigo_setor,
          sm.sector_name,
          CASE WHEN op.data_operacao IS NOT NULL THEN 1 ELSE 0 END as has_data
        FROM date_range dr
        CROSS JOIN (
          SELECT 124 as codigo_setor, 'SANTA MARIA' as sector_name FROM DUAL UNION ALL
          SELECT 239, 'SÃO SEBASTIÃO' FROM DUAL UNION ALL
          SELECT 240, 'GAMA' FROM DUAL UNION ALL
          SELECT 31, 'PARANOÁ' FROM DUAL
        ) sm
        LEFT JOIN oracle_schema.t_operacao_frota op ON (
          op.data_operacao = dr.check_date 
          AND op.codigo_setor = sm.codigo_setor
          AND op.status_registro = 'ATIVO'
        )
      )
      SELECT 
        ...
        sector_name,
        COUNT(*) as total_days,
        SUM(has_data) as days_with_data,
        ROUND((SUM(has_data) / COUNT(*)) * 100, 2) as completeness_percentage
      FROM data_availability
      GROUP BY codigo_setor, sector_name
      ORDER BY codigo_setor
    `,
      { startDate, endDate }
    );

    return this.analyzeCompletenessResults(results);
  }

  /**
   * Valida consistência dos custos médios
   */
  async validateCostConsistency(): Promise<CostValidationReport> {
    const results = await this.oracleConnection.query(`
      WITH cost_analysis AS (
        SELECT 
          codigo_setor,
          tipo_custo,
          AVG(valor_custo / NULLIF(km_dia, 0)) as custo_medio_rs_km,
          STDDEV(valor_custo / NULLIF(km_dia, 0)) as desvio_padrao,
          COUNT(*) as sample_size,
          MIN(valor_custo / NULLIF(km_dia, 0)) as min_custo_km,
          MAX(valor_custo / NULLIF(km_dia, 0)) as max_custo_km
        FROM (
          SELECT 
            op.codigo_setor,
            cs.tipo_custo,
            cs.valor_custo,
            op.km_rodada as km_dia,
            cs.data_custo
          FROM oracle_schema.t_operacao_frota op
          INNER JOIN oracle_schema.t_custos_operacionais cs ON (
            cs.codigo_setor = op.codigo_setor 
          ...
          WHERE op.data_operacao >= ADD_MONTHS(SYSDATE, -12)
            AND cs.tipo_custo IN ('PNEUS', 'PECAS')
            AND op.km_rodada > 0
            AND cs.valor_custo > 0
        )
      ...
      SELECT 
        ...
        max_custo_km,
        CASE 
          WHEN tipo_custo = 'PNEUS' AND custo_medio_rs_km BETWEEN 0.10 AND 2.00 THEN 'NORMAL'
          WHEN tipo_custo = 'PECAS' AND custo_medio_rs_km BETWEEN 0.50 AND 5.00 THEN 'NORMAL'
          WHEN custo_medio_rs_km < 0.05 THEN 'MUITO_BAIXO'
          WHEN custo_medio_rs_km > 10.00 THEN 'MUITO_ALTO'
          ELSE 'ATENCAO'
        END as status_custo,
        CASE 
          WHEN sample_size < 100 THEN 'AMOSTRA_PEQUENA'
          WHEN desvio_padrao > (custo_medio_rs_km * 0.5) THEN 'ALTA_VARIABILIDADE'
          ELSE 'OK'
    ...
    return this.analyzeCostResults(results);
  }
  ...
  async detectDataAnomalies(
    ...
    // Detectar dias com KM zero ou muito alto
    const kmAnomalies = await this.oracleConnection.query(
      `
      SELECT 
        data_operacao,
        codigo_setor,
        km_rodada,
        CASE 
          WHEN km_rodada = 0 THEN 'KM_ZERO'
          WHEN km_rodada > 100000 THEN 'KM_MUITO_ALTO'
          ELSE 'NORMAL'
        END as anomaly_type
      FROM oracle_schema.t_operacao_frota
      WHERE data_operacao BETWEEN TO_DATE(:startDate, 'YYYY-MM-DD') 
                               AND TO_DATE(:endDate, 'YYYY-MM-DD')
        AND codigo_setor IN (124, 239, 240, 31)
        AND (km_rodada = 0 OR km_rodada > 100000)
      ORDER BY data_operacao DESC, codigo_setor
    `,
      { startDate, endDate }
    );

    anomalies.push(...this.processKmAnomalies(kmAnomalies));

    // Detectar custos muito altos em um dia
    const costAnomalies = await this.oracleConnection.query(
      `
      WITH daily_costs AS (
        SELECT 
          data_custo,
          ...
          tipo_custo,
          valor_custo,
          AVG(valor_custo) OVER (
            PARTITION BY codigo_setor, tipo_custo 
            ORDER BY data_custo 
            ROWS BETWEEN 30 PRECEDING AND 1 PRECEDING
          ) as media_30_dias
        FROM oracle_schema.t_custos_operacionais
        WHERE data_custo BETWEEN TO_DATE(:startDate, 'YYYY-MM-DD') 
                              AND TO_DATE(:endDate, 'YYYY-MM-DD')
          AND tipo_custo IN ('PNEUS', 'PECAS')
          AND codigo_setor IN (124, 239, 240, 31)
      )
      SELECT 
        data_custo,
        ...
        valor_custo,
        media_30_dias,
        ROUND((valor_custo / NULLIF(media_30_dias, 0)) * 100, 2) as percentual_variacao
      FROM daily_costs
      WHERE valor_custo > (media_30_dias * 3)  -- Mais que 3x a média
        AND media_30_dias > 0
    ...
    anomalies.push(...this.processCostAnomalies(costAnomalies));
...
interface CostValidationReport {
  isValid: boolean;
  costs: {
    sectorId: number;
    criterionType: 'PNEUS' | 'PECAS';
    averageCostPerKm: number;
    status: 'NORMAL' | 'ATENCAO' | 'MUITO_BAIXO' | 'MUITO_ALTO';
    qualityStatus: 'OK' | 'AMOSTRA_PEQUENA' | 'ALTA_VARIABILIDADE';
  }[];
  warnings: string[];
}

interface AnomalyReport {
  type: 'KM_ZERO' | 'KM_MUITO_ALTO' | 'CUSTO_ANOMALO';
  date: string;
  sectorId: number;
  value: number;
...
### **14.8: Configuração da Entidade SectorEntity**

```sql
-- ============================================================================
-- SCRIPT DE MIGRAÇÃO: Adicionar campo erpId na tabela sectors
-- ============================================================================

-- 1. Adicionar coluna erpId
ALTER TABLE sectors
ADD COLUMN erp_id INTEGER NULL;

-- 2. Comentário da coluna
COMMENT ON COLUMN sectors.erp_id IS 'Código do setor no ERP Oracle para mapeamento';

-- 3. Atualizar registros existentes com os códigos Oracle
UPDATE sectors SET erp_id = 124 WHERE id = 3 AND nome = 'SANTA MARIA';
UPDATE sectors SET erp_id = 239 WHERE id = 4 AND nome = 'SÃO SEBASTIÃO';
UPDATE sectors SET erp_id = 240 WHERE id = 1 AND nome = 'GAMA';
UPDATE sectors SET erp_id = 31  WHERE id = 2 AND nome = 'PARANOÁ';

-- 4. Criar índice para otimizar consultas por erpId
CREATE INDEX idx_sectors_erp_id ON sectors(erp_id) WHERE erp_id IS NOT NULL;

-- 5. Adicionar constraint para garantir unicidade do erpId
ALTER TABLE sectors
ADD CONSTRAINT uk_sectors_erp_id UNIQUE (erp_id);

-- 6. Verificar mapeamento
SELECT
  id as sector_id,
  nome as sector_name,
  erp_id as oracle_code,
  ativo as is_active,
  created_at
FROM sectors
...
```typescript
// Atualização da entidade SectorEntity
@Entity('sectors')
export class SectorEntity {
  @PrimaryGeneratedColumn()
  id: number;
  ...
  // NOVO CAMPO - Código do setor no Oracle ERP
  @Column({ name: 'erp_id', nullable: true, unique: true })
  erpId: number | null;
